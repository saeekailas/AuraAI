...
const VisualGenerator: React.FC<VisualGeneratorProps> = ({ onClose, onAssetGenerated, onLog }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const imageUrl = await geminiService.generateImage(prompt, aspectRatio, isProMode ? 'pro' : 'standard');
      setResult(imageUrl);
      onLog("Neural Matrix Resolved: Asset Generated", "success");
    } catch (err: any) {
      let msg = err.message || "Visual Engine Failure";
      if (msg.includes("QUOTA_EXCEEDED")) {
        msg = "Standard Quota Exceeded. Please enable 'Pro Mode' to use a paid API key for generation.";
      }
      setError(msg);
      onLog(msg, "error");
    } finally {
        ...
        <div className="w-full md:w-1/2 p-10 border-r-2 border-black flex flex-col gap-8 bg-neutral-50 overflow-y-auto">
          <div className="flex justify-between items-start">
            <div>
              ...
              <span className="text-[9px] font-black uppercase text-neutral-400 tracking-[0.3em]">Synthesis_Core_v1.1</span>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-[9px] font-black uppercase tracking-widest ${isProMode ? 'text-indigo-600' : 'text-neutral-400'}`}>Pro_Mode</span>
                <button 
                  onClick={() => setIsProMode(!isProMode)}
                  className={`w-10 h-5 rounded-full relative transition-colors border border-black ${isProMode ? 'bg-indigo-600' : 'bg-neutral-200'}`}
                >
                  <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white border border-black transition-all ${isProMode ? 'left-5' : 'left-1'}`}></div>
                </button>
              </div>
            </div>
            <textarea
            ...
            />
            {isProMode && (
              <p className="text-[9px] text-indigo-600 font-bold uppercase leading-tight italic">
                * Pro Mode uses gemini-3-pro and requires your own selected paid API key.
              </p>
            )}
          </div>
          ...
          <div className="mt-auto pt-8">
            ...
            <button
              ...
              className={`w-full py-5 text-[12px] font-black uppercase tracking-[0.5em] transition-all border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] active:translate-x-[4px] active:translate-y-[4px] active:shadow-none ${
                isGenerating ? 'bg-neutral-100 text-neutral-400' : 
                isProMode ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-emerald-500 text-white hover:bg-emerald-600'
              }`}
            >
              {isGenerating ? 'Computing Pixels...' : `Synthesize ${isProMode ? 'HD' : 'Visual'}`}
            </button>
          </div>
        </div>
